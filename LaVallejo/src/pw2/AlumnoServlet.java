			...
			if (personas.isEmpty()) {
				...
				if (req.getParameter("action") == null) {
					...
					String busqueda = "select from "+Persona.class.getName()
							+" where categoria=='alumno' && "+req.getParameter("busqueda")+
					...
					RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/listadoalumno.jsp");
					dispatcher.forward(req, resp);
				} else if (req.getParameter("action").equals("eliminar")) {
					String busqueda = "select from "+Persona.class.getName()
							+" where categoria=='alumno' && idPersona=='"+
							req.getParameter("id")+"'";
					final Query q = pm.newQuery(busqueda);
					q.deletePersistentAll();
					getServletContext().setAttribute("message", "Alumno eliminado correctamente");
					RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/menualumno.jsp");
					dispatcher.forward(req, resp);
					...
					if (result.isEmpty()) {
						try{
							...
							try{
								...
								getServletContext().setAttribute("message", "se guardó alumno correctamente");
								RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/menualumno.jsp");
								dispatcher.forward(req, resp);
							}catch(Exception e){
						...
						getServletContext().setAttribute("message", "El alumno ya se registro con anterioridad");
						RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/menualumno.jsp");
						dispatcher.forward(req, resp);
					}
